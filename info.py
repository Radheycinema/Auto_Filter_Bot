...
API_ID = int(environ.get('API_ID', '8049727855')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '76746bb61cd52e2a0842def933534a10')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "8049727855:AAFdHxubdFPX3gta1A3gW2gO3qDYxfJOYJ8")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7055240040').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002694399477').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003064920392'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002915027731'))  # Bin channel id (make sure bot is admin)
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003099409176')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002988330517')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/+VWQ-3mip3CxhODM9')  # Support group link (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Radhey:Radhey@radhey.lxzme.mongodb.net/?retryWrites=true&w=majority&appName=Radhey")  # MongoDB URI for the database
DATABASE_NAME = environ.get('DATABASE_NAME', "Radhey") # Database name (default: cluster)
